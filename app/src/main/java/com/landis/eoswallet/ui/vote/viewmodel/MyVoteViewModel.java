...
public class MyVoteViewModel extends BaseViewModel {

    public final ObservableList<Object> myVoteList = new ObservableArrayList<>();
        ...
        HttpManage.apiService.getVoteNodes("eosio", "eosio", "bps", true, 50).compose(RxUtils.applySchedulers()).subscribe(voteNodesList -> {
            ...
            if (!voteNodesList.rows.isEmpty()) {
                Collections.sort(voteNodesList.rows, new Comparator<VoteNodesList.VoteNodes>() {
                    @Override
                    public int compare(VoteNodesList.VoteNodes voteNodes,
                                       VoteNodesList.VoteNodes t1) {
                        try {
                ...
                for (int i = 0; i < voteNodesList.rows.size(); i++) {
                    ...
                    voteNodesList.rows.get(i).amount =
                            voteViewModel.getAmount(voteNodesList.rows.get(i).name);
    ...
    private void getMyVote(List<VoteNodesList.VoteNodes> voteNodes) {
        List<VoteNodesList.VoteNodes> voteNodesList = new ArrayList<>();
        HttpManage.apiService.getVoteRows(SPUtils.getInstance().getString(SpConst.WALLET_NAME),
                "eosio",
                "votes",
                true, 50)
                ...
                for (int i = 0; i < voteNodes.size(); i++) {
                    for (int j = 0; j < userVotes.size(); j++) {
                        if (userVotes.get(j).bpname.equals(voteNodes.get(i).name)) {
                            ...
                            //格式化赎回票数
                            Float unStaking = Float.valueOf(userVotes.get(j).unstaking.replace(" " +
                                    "EOS", ""));
                            if (unStaking > 0) {
                            ...
                            //格式化投票数
                            String formatVoteStaked =
                                    numberFormat.format(Float.valueOf(userVotes.get(j).staked.replace(" EOS", "")));
                            voteNodes.get(i).vote_staked = formatVoteStaked;
                            voteNodes.get(i).claim_balance =
                                    fourdf.format(calculation(voteNodes.get(i), userVotes.get(j)));
                            voteNodesList.add(voteNodes.get(i));
                        }
                    }
                }
            }
            if (null != myVoteList && myVoteList.size() > 0) {
                myVoteList.clear();
            }
            myVoteList.addAll(voteNodesList);
            if (myVoteList.size() > 0) {
    ...
    private Float calculation(VoteNodesList.VoteNodes voteNodes, UserVoteList.UserVote userVote) {
        ...
        //我的最新票龄
        Float LatestTicketAge = userVote.voteage + Float.valueOf(userVote.staked.replace("EOS",
                "").trim()) * voteageUpdateHeight;
        //节点最新票龄
        long total_voteage = voteNodes.total_voteage;
        long total_staked = voteNodes.total_staked;
        long totalVoteageHeight =
                total_voteage + total_staked * (lastIrreversibleBlockNum - voteNodes.voteage_update_height);
        float claimBalance =
                (LatestTicketAge / totalVoteageHeight) * Float.valueOf(voteNodes.rewards_pool.replace("EOS", "").trim());
